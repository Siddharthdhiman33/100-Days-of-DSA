...
int main() {
    int p, q;
    printf("Enter no. of logs of user arrival time from first server: ");
    scanf("%d", &p);
    int log1[p];
    for (int i = 0; i < p; i++) {
        scanf("%d", &log1[i]);
    }
    printf("Enter no. of logs of user arrival time from second server: ");
    scanf("%d", &q);
    int log2[q];
    for (int i = 0; i < q; i++) {
        scanf("%d", &log2[i]);
    ...
    int i = 0, j = 0;
    printf("Single chronological log: ");    
    while (i < p && j < q) {
        if (log1[i] <= log2[j]) {
            printf("%d ", log1[i]);
            i++;
